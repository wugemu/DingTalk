...
var GLOBAL = {
	...
	ORIGIN_LOG_DEBUG : false,
	SCALE_DEBUG : false,
	QUOTE_DEBUG : false
};

// 原来的console函数，下面的会对window.console进行覆盖，所以这里先保存一份原来的副本
var originConsole = window.console;
...
(function(_aoNS) {
	...
	_aoNS.parentContains = function(clazzName, _aoDom) {
	    if (!_aoDom) {
	        return false;
	    }
	    var parentNode = _aoDom.parentNode;
	    while (parentNode != null && parentNode.tagName != "BODY") {
	        var tagName = parentNode.tagName;
	        if (tagName == clazzName) {
	            return true;
	        }
...
(function(_aoNS) {
	...
	function _console(_anType, _asText, _abIsLogFile) {
		...
		if (_abIsLogFile) {
			...
			App.log(_asText);
		} else if (GLOBAL.ORIGIN_LOG_DEBUG && _asText) {
		    originConsole.log(_asText);
		}

		//显示在页面
		if (GLOBAL.PAGE_LOG_DEBUG) {
			var _sId = "eEmAilcOnsoLe",
